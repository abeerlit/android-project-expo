...
/**
 * User-tap trampoline for heads-up Answer.
 *
 * Must run over the lock screen so the tap delivers ANSWER instead of only
 * opening the app. Sends ANSWER immediately, then waits for SIP 200 OK
 * ([ACTION_UPDATE_UI]) before starting MainActivity so headless JS is not
 * killed mid-accept (that race drops later background INVITEs).
 */
