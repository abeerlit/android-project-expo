...
class IncomingCallFullScreenActivity : AppCompatActivity() {
    ...
    private fun handleAnswer(callUuid: String) {
        ...
        val keyguardManager = getSystemService(Context.KEYGUARD_SERVICE) as KeyguardManager
        if (keyguardManager.isDeviceLocked) {
            // Locked: launch MainActivity after delay so app is in back stack when user unlocks
            Handler(Looper.getMainLooper()).postDelayed({
                launchMainActivity(callUuid)
            }, 2500)
        } else {
            // Unlocked: launch MainActivity so user lands in call screen (no tap-on-notification needed)
            Handler(Looper.getMainLooper()).postDelayed({
                launchMainActivity(callUuid)
            }, 1500)
        }
        ...
        if (!keyguardManager.isDeviceLocked) {
            ...
            keyguardManager.requestDismissKeyguard(this, object : KeyguardManager.KeyguardDismissCallback() {
                ...
                override fun onDismissError() {
                    ...
                    // Keep call ongoing in notification/native UI, but never show incoming UI again.
                    finish()
                }
                override fun onDismissCancelled() {
                    Log.d(TAG, "onDismissCancelled")
                    // User backed out of keyguard prompt; dismiss incoming full-screen UI.
                    finish()
                }
            })
        }
    ...
    inner class TelecomBroadcastReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            when (intent.action) {
                ...
                "ACTION_UPDATE_UI" -> {
                    // Call is now connected/ongoing. Dismiss incoming full-screen UI so it
                    // doesn't keep showing Answer/Reject when user canceled keyguard prompt.
                    Log.d(TAG, "ACTION_UPDATE_UI, dismissing incoming full-screen UI")
                    finish()
                }
