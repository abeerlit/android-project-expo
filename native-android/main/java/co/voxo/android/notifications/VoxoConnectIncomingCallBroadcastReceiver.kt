...
class VoxoConnectIncomingCallBroadcastReceiver : BroadcastReceiver() {
            ...
            actionPerformed == "END_AND_ACCEPT" && callUuid != null -> {
                try {
                    ...
                    if (fromHeadsUp) {
                        ...
                        if (isLocked) {
                            scheduleMainActivityDelayed(context, callUuid, 2500L)
                        } else {
                    ...
                    when (actionPerformed) {
                        "ANSWER" -> {
                            if (fromHeadsUp && callUuid != null) {
                                val keyguard = context.getSystemService(Context.KEYGUARD_SERVICE) as KeyguardManager
                                val isLocked = keyguard.isDeviceLocked
                                Log.d(TAG, "ANSWER fromHeadsUp=true isDeviceLocked=$isLocked -> ${if (isLocked) "scheduleMainActivityDelayed(2500)" else "pendingHeadsUpUnlockedCallUuid (launch after SIP answer)"}")
                                if (isLocked) {
                                    scheduleMainActivityDelayed(context, callUuid, 2500L)
                                } else {
                                    // Do NOT launch MainActivity here. Set flag so HandleSipCallHeadlessTask launches
                                    // it after CALL_ANSWERED (SIP 200 OK sent). Prevents MainActivity from competing
                                    // with headless JS and delaying the answer.
                                    pendingHeadsUpUnlockedCallUuid = callUuid
                                }
                                return
                            }
    ...
    private fun scheduleMainActivityDelayed(context: Context, callUuid: String, delayMs: Long) {
        Log.d(TAG, "scheduleMainActivityDelayed: callUuid=$callUuid delayMs=$delayMs")
        Handler(Looper.getMainLooper()).postDelayed({
            Log.d(TAG, "scheduleMainActivityDelayed: runnable executing, starting MainActivity")
            try {
                ...
                Log.d(TAG, "scheduleMainActivityDelayed: startActivity completed")
            } catch (e: Exception) {
                Log.e(TAG, "scheduleMainActivityDelayed: startActivity FAILED", e)
            }
        }, delayMs)
    }
